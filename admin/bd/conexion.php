<?php
     ...
     class BD{
        ...
        public function getPerrosEnTramiteAdopcion(){
            $sql = "SELECT DISTINCT perro.nChip,perro.nombrePerro,perro.fechaNacimiento,perro.fechaEntrada,perro.idperrera,perro.peso,perro.idRaza
            FROM perro
            INNER JOIN adopcion_perros
            ON perro.nChip = adopcion_perros.nChip where
    ...
    public function getIdRolUserByDni($dni){
        $sql="SELECT idRol FROM usuario WHERE dni = '$dni'";
        $consulta = $this->conexion->prepare($sql);
        $consulta->execute();
        $data = $consulta->fetch(PDO::FETCH_LAZY);
        return @$data['idRol'];
